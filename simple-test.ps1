...
# Version đơn giản để tránh lỗi PowerShell phức tạp

Write-Host "🚀 Simple Performance Test - Game Optimizations" -ForegroundColor Cyan
...
Write-Host "4. Running performance test (15 seconds)..." -ForegroundColor Yellow

$logFile = "simple_test_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"
...
# Wait 15 seconds
Write-Host "   ⏱️ Running for 15 seconds..." -ForegroundColor Cyan
...
if (Test-Path $logFile) {
    ...
    if ($frames -gt 0) {
        ...
        $avgFrameTime = [math]::Round(15000 / $frames, 2) # 15 seconds in ms
