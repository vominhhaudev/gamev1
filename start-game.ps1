# 🚀 GAMEV1 - Ultimate Startup Script
# ===================================
...
try {
    ...
    try {
        while ($true) {
            ...
            $processes = @($workerProcess, $clientProcess)
            if ($gatewayProcess) { $processes += $gatewayProcess }
            if ($pbProcess) { $processes += $pbProcess }
            ...
            if ($running -eq 0) {
                Write-Host ""
                Write-Host "⚠️  All services have stopped unexpectedly!" -ForegroundColor Yellow
                break
            }
        }
    }
